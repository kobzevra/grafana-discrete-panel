...
const externals: Configuration['externals'] = [
  ...
  ({ request }: ExternalItemFunctionData, callback: (error?: Error, result?: string) => void) => {
    if (request?.startsWith('grafana/')) {
      return callback(undefined, request.slice('grafana/'.length));
    }
    callback();
  },
];
...
export default (env: Record<string, unknown>): Configuration => ({
  ...
  plugins: [
    new CopyWebpackPlugin({
      patterns: [
        { from: 'plugin.json', to: 'plugin.json', toType: 'file' },
        { from: 'img', to: 'img', noErrorOnMissing: true },
        { from: '../README.md', to: 'README.md', toType: 'file', noErrorOnMissing: true },
